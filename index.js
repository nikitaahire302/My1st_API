...
// Middleware to check master key
const checkMasterKey = (req, res, next) => {
  const key = req.header('x-master-key');
  if (key === masterKey) {
    next();
  } else {
    res.status(403).send('Forbidden: Invalid master key');
  }
};
...
// 4. POST a new joke
app.post("/jokes", checkMasterKey, (req, res) => {
  const { jokeText, jokeType } = req.body;
  if (!jokeText || !jokeType) {
    return res.status(400).send("Bad Request: Missing jokeText or jokeType");
  }
  const newJoke = {
...
// 5. PUT a joke
app.put("/jokes/:id", checkMasterKey, (req, res) => {
  const joke = jokes.find(j => j.id === parseInt(req.params.id));
  if (joke) {
    const { jokeText, jokeType } = req.body;
...
// 6. PATCH a joke
app.patch("/jokes/:id", checkMasterKey, (req, res) => {
  const joke = jokes.find(j => j.id === parseInt(req.params.id));
  if (joke) {
    if (req.body.jokeText) joke.jokeText = req.body.jokeText;
    if (req.body.jokeType) joke.jokeType = req.body.jokeType;
    res.json(joke);
  } else {
    res.status(404).send("Joke not found");
  }
...
app.delete("/jokes/:id", checkMasterKey, (req, res) => {
...
// 8. DELETE All jokes
app.delete("/jokes", checkMasterKey, (req, res) => {
  jokes = [];
  res.status(204).send();
});
